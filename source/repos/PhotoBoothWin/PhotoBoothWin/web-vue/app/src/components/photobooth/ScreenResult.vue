...
const { getQrCodePageBackground, getQrCodePagePrintButton, projectVariant } = useProjectVariant()
const resultScreenStyle = computed(() => {
  void projectVariant.value
  return { backgroundImage: `url('${getQrCodePageBackground()}')` }
})
const printButtonUrl = computed(() => {
  void projectVariant.value
  return getQrCodePagePrintButton()
})
...
<template>
  <div class="screen screen--result" role="region" aria-label="結果畫面" :style="resultScreenStyle">
    <div class="result-wrap">
      ...
      <div class="btns">
        ...
        <button type="button" class="print-btn" @click="onPrint">
          <img :src="printButtonUrl" alt="確認儲存完畢" />
        </button>
      </div>
      </div>
    </div>
