...
const { getIdleBasePath, getIdleCoverSlides, projectVariant } = useProjectVariant()
const basePath = computed(() => getIdleBasePath())
/** 輪播圖（相對 basePath）；2 號專案使用 IdlePage/cover/2 */
const slideImages = computed(() => {
  void projectVariant.value
  return getIdleCoverSlides()
})
...
function getSlideUrl(img: string): string {
  return img.startsWith('/') ? img : `${basePath.value}/${img}`
}

/** 無輪播時用的靜態背景：用第一張圖 */
const staticBackgroundUrl = computed(() => {
  void projectVariant.value
  const slides = slideImages.value
  return slides[0] ? getSlideUrl(slides[0]) : ''
})
...
function startCarousel() {
  const slides = slideImages.value
  if (!isCarousel.value || slides.length <= 1) return
