...
/** 左側面板 style（left 依拍照外框動態計算；濾鏡欄出現時會重算 panel 總寬） */
const leftPanelStyle = computed(() => ({ left: `${leftPanelLeftPx.value}px` }))
...
const shootTexUrl = computed(() => {
  ...
  return getShootTexUrl(n)
})
const shootChooseTextUrl = computed(() => {
  void projectVariant.value
  return getShootPageAsset('choosetext.png')
})
const shootReadTextUrl = computed(() => {
  void projectVariant.value
  return getShootPageAsset('Readtext.png')
})
          ...
          <div class="title-align-wrap">
            <div class="title-align-inner">
              ...
              <img
                ...
                :src="shootChooseTextUrl"
                alt="選擇濾鏡"
              />
              <img
                v-else
                class="shoot-read-title-img"
                :src="shootReadTextUrl"
                alt="請看上方鏡頭"
              />
            </div>
          </div>
...
.screen--shoot {
  ...
  .title-and-picture-column {
    ...
    padding-bottom: 4px;
    /* 整欄（標題＋預覽＋按鈕）略往右移 */
    margin-left: clamp(32px, 13.5vw, 450px);
  ...
  .btns {
    ...
    .again-btn {
      background-image: var(--shoot-btn-reshoot);
    }
