...
const projectSwitchMessage = ref('')

const { showScreen, resetSession } = usePhotobooth()
const { toggleProjectVariant, isProject2 } = useProjectVariant()
...
function close() {
  ...
  error.value = ''
  uploadMessage.value = ''
  projectSwitchMessage.value = ''
  resetTap()
}

function onSwitchProject() {
  const next = toggleProjectVariant()
  projectSwitchMessage.value = next === '2' ? '已切換至 2 號專案' : '已切換至 1 號專案'
  setTimeout(() => { projectSwitchMessage.value = '' }, 3000)
}
...
<template>
  ...
  <div v-if="open" class="secret-overlay" role="dialog" aria-label="管理登入">
    ...
    <div v-else class="secret-menu">
      ...
      <button type="button" class="menu-btn" @click="onSwitchProject">
        切換專案（目前：{{ isProject2 ? '2' : '1' }} 號 → {{ isProject2 ? '1' : '2' }} 號）
      </button>
      ...
      <p v-if="projectSwitchMessage" class="secret-msg">{{ projectSwitchMessage }}</p>
